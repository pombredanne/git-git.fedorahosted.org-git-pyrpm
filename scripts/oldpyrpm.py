...
# - check more tags to have the same length (file-related tags)
# - check for #% in spec files
# - verifyindex handling
...
# - Can hardlinks go into a new rpm tag?
...
class ReadRpm:
    ...
    def getDeps(self, name, flags, version):
        ...
        if v == None:
            v = None*len(n)
        if self.verify and (len(n) != len(f) or len(f) != len(v)):
            self.printErr("wrong length of deps for %s" % name)
        return zip(n, f, v)

    def getProvides(self):
        provs = self.getDeps("providename", "provideflags", "provideversion")
        # this is only needed for some compat rpms in AS2.1 and earlier:
        mydep = (self["name"], RPMSENSE_EQUAL, self.getEVR())
        if mydep not in provs:
            if self.verify and self.strict:
                self.printErr("no provides for own rpm package")
            provs.append(mydep)
        return provs
    ...
    def getTriggers(self):
        ...
        if index == None:
            ...
            if self.verify and len(deps) != len(index):
                self.printErr("wrong triggers")
            scripts = [ scripts[ti] for ti in index ]
            progs = [ progs[ti] for ti in index ]
        d2 = []
        for (n, f, v) in deps:
            d2.append( (n, f, v, progs.pop(0), scripts.pop(0)) )
        return d2
    ...
    def doVerify(self):
        ...
        fileflags = self["fileflags"]
        for i in xrange(len(filenames)):
            # This might be only valid for self.strict:
