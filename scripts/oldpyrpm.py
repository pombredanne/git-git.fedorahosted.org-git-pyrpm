...
# - mkstemp implementation for python < 2.3, remove mktmp()
...
import os, os.path, sys, md5, sha, pwd, grp, zlib, time, errno
from struct import unpack
from commands import getoutput
from types import StringType
from tempfile import mkstemp, _get_candidate_names, TMP_MAX
...
def mkstemp_symlink(dir, pre, symlinkfile):
    ...
    for _ in xrange(TMP_MAX):
        name = names.next()
        filename = os.path.join(dir, pre + name)
        try:
...
def mktmp(path, prefix):
    """Create a temporary filename in "path" with a filename
       starting with "prefix". """
    if not path.endswith("/"):
        path = path + "/"
    path = path + prefix
    # XXX we should add the current time and divide by some prime ??
    # XXX only 4000 path names are not a real lot, this can be an easy DOS
    pid = os.getpid()
    for d in xrange(4000):
        tmp = path + str(pid + d)
        if not os.path.lexists(tmp):
            return tmp
    raise IOError, "Cannot create temporary filename"
...
def setPerms(filename, uid, gid, mode, mtime, owner):
    if owner:
...
class ReadRpm:
    ...
    def extractCpio(self, filedata, data, filenamehash, devinode, filenames):
        ...
        if S_ISREG(mode):
            di = devinode.get((dev, inode))
            if di == None or data:
                (fd, tmpfilename) = mkstemp(dir=dirname, prefix=filename + ".")
                os.write(fd, data)
                os.close(fd)
                setPerms(tmpfilename, uid, gid, mode, mtime, self.owner)
                os.rename(tmpfilename, filename)
                if di:
                    di.remove(i)
                    for j in di:
                        fn2 = "%s%s" % (self.buildroot, filenames[j])
                        makeDirs(os.path.dirname(fn2))
                        unlinkFile(fn2)
                        try:
                            os.link(filename, fn2)
                        except:
                            dirname = os.path.dirname(fn2)
                            (fd, tmpfilename) = mkstemp(dir=dirname, prefix=fn2 + ".")
                            os.write(fd, data)
                            os.close(fd)
                            setPerms(tmpfilename, uid, gid, mode, mtime, self.owner)
            ...
            makeDirs(filename)
            setPerms(filename, uid, gid, mode, None, self.owner)
        elif S_ISLNK(mode):
            #if os.path.islink(filename) \
            #    and os.readlink(filename) == linkto:
            #    return
            tmpfile = mkstemp_symlink(dirname, filename, linkto)
            setPerms(tmpfile, uid, gid, None, None, self.owner)
            os.rename(tmpfile, filename)
        elif S_ISFIFO(mode):
            tmpfile = mkstemp_mkfifo(dirname, filename)
            setPerms(tmpfile, uid, gid, mode, mtime, self.owner)
            os.rename(tmpfile, filename)
        elif S_ISCHR(mode) or S_ISBLK(mode):
            tmpfile = mkstemp_mknod(dirname, filename, mode, rdev)
            setPerms(tmpfile, uid, gid, mode, mtime, self.owner)
            os.rename(tmpfile, filename)
        elif S_ISSOCK(mode):
...
def extractRpm(filename, buildroot, owner=None):
    ...
    if not rpm.issrc and os.geteuid() == 0:
        owner = 1
    rpm.owner = owner
    if owner:
...
def explodeFile(filename, dirname):
