...
# - mkstemp implementation for python < 2.3
...
import os, os.path, sys, md5, sha, pwd, grp, zlib, time
...
if sys.version_info >= (2, 3):
    ...
    def mkstemp(dir=None, prefix=None):
        pass
...
class Uid(UGid):
    def transform(self, buildroot):
        ...
        if buildroot or not os.path.isfile(buildroot + "/sbin/ldconfig"):
            uidhash = parseFile(buildroot + "/etc/passwd")
            for uid in self.ugid.keys():
...
class Gid(UGid):
    def transform(self, buildroot):
        ...
        if buildroot or not os.path.isfile(buildroot + "/sbin/ldconfig"):
            gidhash = parseFile(buildroot + "/etc/group")
            for gid in self.ugid.keys():
...
class ReadRpm:
    ...
    def extractCpio(self, filedata, data, filenamehash, devinode):
        (filename, inode, mode, nlink, mtime, filesize, dev, rdev) = filedata
        i = filenamehash.get(filename)
        if i == None:
            self.printErr("cpio file %s not in rpm header" % filename)
        ...
        if self.owner:
            uid = self.uid.ugid[self["fileusername"][i]]
            gid = self.gid.ugid[self["filegroupname"][i]]
        filename = "%s%s" % (self.buildroot, filename)
        if S_ISREG(mode):
            makeDirs(filename)
            (fd, tmpfilename) = mkstemp(dir=os.path.dirname(filename),
                prefix=filename + ".")
            os.write(fd, data)
            os.close(fd)
            if self.owner:
                os.chown(tmpfilename, uid, gid)
            os.chmod(tmpfilename, mode & 07777)
