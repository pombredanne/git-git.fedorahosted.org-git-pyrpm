...
try:
    ...
    class Partition(dict):
        nativeType = {
            ...
            0x08: "AIX",               # AIX boot (AIX -- PS/2 port) or
                                       # SplitDrive
        ...
        PARTITION_RAID = parted.PARTITION_RAID
        PARTITION_LVM = parted.PARTITION_LVM
        ...
        def __getitem__(self, item):
            if item == "name":
                ...
                if self.ped_partition.get_flag(parted.PARTITION_RAID) == 1:
                    return "raid"
                elif self.ped_partition.get_flag(parted.PARTITION_LVM) == 1:
                    return "lvm"
                elif self.ped_partition.fs_type and \
                         self.ped_partition.fs_type.name:
                    if self.ped_partition.fs_type.name == "linux-swap":
                        return "swap"
    ...
    class Disk(dict):
        ...
        def open(self, filename, as_image=0):
            ...
            if stat.S_ISBLK(mode) and not as_image:
                ...
                log.error("Unsupported device type for '%s'.", device)
                return
        ...
        def reload(self):
            ...
            while ped_partition:
                ...
                # all other partitions
                    ...
                    if self.has_key("image"):
                        if self.alloc_loop:
                            device = loop.LOOP.findUnused()
                            if device:
                                l = loop.LOOP(device)
                                offset = self.partition[num]["start"] * \
                                         self["sector_size"]
                                if not l.setup(self["device"], offset=offset):
                                    device = None
                                else:
                                    l.setSizelimit(\
...
#        k.append("type")
        ...
        def new_disklabel(self, label):
            if not label in Disk.diskType:
                log.error("Disk label '%s' is not supported.", label)
                return
